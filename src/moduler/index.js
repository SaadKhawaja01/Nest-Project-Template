...
(async () => {
    ...
    const paths = await Promise.all([
        makeDir(`./src/app/${camelize(entity)}`),
    ]);
    ...
    if (content.includes(`export * from './${camelize(entity)}/${camelize(entity)}.dto';`) === false) {
        fs.appendFileSync('./src/app/index.dto.ts', `export * from './${camelize(entity)}/${camelize(entity)}.dto';\n`)
    }

    content = fs.readFileSync('./src/app/app.module.ts', 'utf-8');
    if (content.includes(`import { ${entity}Module } from './${camelize(entity)}/${camelize(entity)}.module'`) === false) {
        content = content.replaceAll('/*NextModule*/', `,${entity}Module/*NextModule*/`)
        content = content.replaceAll('/*NextModulePath*/', `import { ${entity}Module } from './${camelize(entity)}/${camelize(entity)}.module'\n/*NextModulePath*/`)
        fs.writeFileSync('./src/app/app.module.ts', content)
    }

    content = fs.readFileSync('./src/core/config/database.config.ts', 'utf-8');

    if (content.includes(`import { ${entity} } from 'src/app/${camelize(entity)}/${camelize(entity)}.entity'`) === false) {
        content = content.replaceAll('/*NextEntity*/', `,${entity}/*NextEntity*/`)
        content = content.replaceAll('/*NextEntityPath*/', `import { ${entity} } from 'src/app/${camelize(entity)}/${camelize(entity)}.entity'\n/*NextEntityPath*/`)
